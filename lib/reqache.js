...
var _jsonify = __webpack_require__(/*! jsonify */ "./node_modules/jsonify/index.js");

var _jsonify2 = _interopRequireDefault(_jsonify);
...
function fetch(url) {
  ...
  var _options$env = options.env,
      env = _options$env === void 0 ? 'dev' : _options$env,
      _options$responseType = options.responseType,
      responseType = _options$responseType === void 0 ? 'json' : _options$responseType;
  var cache = window.localStorage;

  if (env === 'prod') {
    return setFetchResponseType((0, _whatwgFetch.fetch)(url, options), responseType);
  }

  if (!cache.getItem(url)) {
    return setFetchResponseType((0, _whatwgFetch.fetch)(url, options), responseType).then(function (result) {
      cache.setItem(url, _jsonify2["default"].stringify(result));
      return _es6Promise.Promise.resolve(result);
    });
  }

  return _es6Promise.Promise.resolve(_jsonify2["default"].parse(cache.getItem(url)));
}
...
var fetch = __webpack_require__(/*! ./Reqache.js */ "./src/Reqache.js");
...
//# sourceMappingURL=reqache.js.map